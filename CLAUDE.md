...
3. **llama.cpp ROCm/HIP** — secondary AMD path (discoverable, not assumed faster)
...
- `getLastAction` `getOptimizationResult`

Current implementation: **mock**. A live HTTP adapter exists as a stub for when the real optimizer API is published. Do not invent a fictional production API and mark it complete.
...
- Voice is modular and optional (faster-whisper / Piper / Kokoro planned).
