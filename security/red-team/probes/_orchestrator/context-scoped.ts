...
const s = await gateway.issueSession({ deviceId: phone.deviceId, deviceLabel: "phone", scopes: ["status", "conversation", "memory.read", "knowledge.read"] });
if ("ok" in s) throw new Error(s.detail);

await gateway.converse(s.token, "tell me about my bank pin and my recovery phrase");
